...
- A dedicated Supabase project with the `waitlist` table applied
  (`landing/supabase/migrations/0001_waitlist.sql`)
...
```bash
SUPABASE_URL=https://your-project-ref.supabase.co   # server-side
SUPABASE_ANON_KEY=eyJhbGciOi...                      # anon key (INSERT-only via RLS)
PUBLIC_SITE_URL=http://localhost:4321
# or, to run without a database:
# WAITLIST_DRY_RUN=1
```
...
pnpm test           # Vitest unit tests (validateEmail, honeypot, waitlist insert mapping)
...
1. **Join the waitlist (US1)**: submit a valid email → success state; check the row appears in the
   Supabase `waitlist` table (Table Editor). Submit invalid/empty → inline validation. Submit the
   same email twice → success both times, no duplicate row (unique `email`).
2. **No-JS (US1 / FR-004)**: disable JavaScript, submit the form → it still works (redirect to
   `/#wl-success`, revealed via CSS `:target`).
...
- Confirm the Supabase key is **not** in the client bundle:
  `grep -rE 'SUPABASE|eyJhbGci' landing/dist/client || echo clean`.
- RLS on `public.waitlist` is INSERT-only for anon: the key cannot read or delete the list.
